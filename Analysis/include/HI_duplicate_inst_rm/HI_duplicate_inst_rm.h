#ifndef _HI_DUPLICATE_INST_RM
#define _HI_DUPLICATE_INST_RM
...
class HI_duplicate_inst_rm : public FunctionPass
{
public:
    HI_duplicate_inst_rm(const char *RemoveLog_Name) : FunctionPass(ID)
    {
        ...
        Loop_Counter = 0;
        callCounter = 0;
        RemoveLog = new raw_fd_ostream(RemoveLog_Name, ErrInfo, sys::fs::OF_None);
        tmp_stream = new raw_string_ostream(tmp_stream_str);
    } // define a pass, which can be inherited from ModulePass, LoopPass, FunctionPass and etc.

    ~HI_duplicate_inst_rm()
    {
    ...
    {
        print_status("Initilizing HI_duplicate_inst_rm pass.");
