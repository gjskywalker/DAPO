...
void HI_qor_estimator::findMemoryDeclarationin(Function *F,
                                                                     bool isTopFunction)
{
...
void HI_qor_estimator::TraceAccessForTarget(Value *cur_node, Value *ori_node)
{
...
HI_qor_estimator::scheduleBRAMAccess(
    Instruction *access, BasicBlock *cur_block,
    HI_qor_estimator::timingBase cur_Timing)
{
...
HI_qor_estimator::timingBase
HI_qor_estimator::handleBRAMAccessFor(
...
bool HI_qor_estimator::checkBRAMAvailabilty(
...
void HI_qor_estimator::insertBRAMAccessInfo(Value *target,
...
HI_qor_estimator::BRAM_MUX_Evaluate()
{
...
HI_qor_estimator::resourceBase
HI_qor_estimator::get_BRAM_Num_For(AllocaInst *alloca_I)
{
...
// get the number of BRAMs which are needed by the alloca instruction
HI_qor_estimator::resourceBase
HI_qor_estimator::get_BRAM_Num_For(int width, int depth)
{
    resourceBase res(0, 0, 0, 0, clock_period);
    if (depth <= 0)
        return res;
...
// AliasResult HI_qor_estimator::HI_AAResult::alias(const MemoryLocation &LocA,
...
bool HI_qor_estimator::hasRAWHazard(Instruction *loadI, int cycle)
{
...
bool HI_qor_estimator::isNotPoison(Instruction* instr) {
    llvm::Metadata* md = instr->getMetadata("llvm.mem.parallel_loop_access.not_unsafe");
    return md != nullptr;
}
// Value *HI_qor_estimator::getTargetFromInst(Instruction *accessI)
// {
