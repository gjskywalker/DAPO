void dut(const int word_buffer_m[8][10], const int old_word_buffer_m[8][10], const bool lb[8], const bool rb[8], int line_buffer_m[8][3][10], const int conv_params_m[3][3], int conv_out_buffer_m[64], const int log_width) {
    // slices_per_line = width / BANK_WIDTH
    const int slices_per_line = 1 << (log_width - 3);

    // Convolution
    for (int bank = 0; bank < 8; bank++) { // {"line_buffer_m": {"dim": 1, "type": "cyclic", "factor": 0}, "conv_out_buffer_m": {"dim": 1, "type": "block", "factor": 8}}
        for (int cc = 0; cc < 8; cc++) { // {"line_buffer_m": {"dim": 3, "type": "cyclic", "factor": 0}, "conv_out_buffer_m": {"dim": 1, "type": "cyclic", "factor": 0}}
            int sum = 0;
