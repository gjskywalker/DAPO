...
/*
    Current Program:random3 -- Cycles:349002 -- Passes:[6, 13, 6, 9] instcombine licm instcombine correlated-propagation
*/
    ...
    #pragma HLS unroll factor=2
    {
        ...
        else
        {
            ...
            for (k = 1; k <= 12; k++)
            {
                ...
                if (((k % 3) == 1) && (k != 1))
                {   // unroll the innner loop, unroll is even not useful here
                    // Fully unrolled (j = 1 .. 2) former inner loop
                    // Iteration j = 1
                    if (beta < 0)
                    {
                        x_new = x - (y >> k);
