                                              ...
                                              bool NonNegative)
{
    ...
    if (BO->getOpcode() == Instruction::Or && !haveNoCommonBitsSet(LHS, RHS, DL))
        return false;
...
APInt HI_ConstantOffsetExtractor::findInEitherOperand(BinaryOperator *BO, bool SignExtended, bool ZeroExtended)
{
    ...
    if(BO->getOperand(0) != BO)
        ConstantOffset = find(BO->getOperand(0), SignExtended, ZeroExtended,
                                /* NonNegative */ false);
...
Value *HI_ConstantOffsetExtractor::applyExts(Value *V)
{
    ...
    {
        if (ConstantInt *C = dyn_cast<ConstantInt>(Current))
        {
            ...
            // errs() << *(*I) << "\n";
            // errs() << (*I)->getOpcode() << "\n";
            // if((*I)->getOpcode() == Instruction::SExt || (*I)->getOpcode() == Instruction::ZExt || (*I)->getOpcode() == Instruction::Trunc)
            // {
            //     if(ConstantInt *tmpC = dyn_cast<ConstantInt>((*I)->getOperand(0)))
            //         Current = tmpC;
            // }
            
            /*
                This is the new API to cast a constantInt in llvm; And this will make the compile time for some cases become longer;
            */
            if((*I)->getOpcode() == Instruction::SExt)
            {
                auto tmp = C->getSExtValue();
                Current = C->getSigned((*I)->getType(), tmp);
            }
            else if ((*I)->getOpcode() == Instruction::ZExt)
            {
                auto tmp = C->getZExtValue();
                Current = C->get((*I)->getType(), tmp);
            }
        }
        else
        {
...
Value *HI_ConstantOffsetExtractor::distributeExtsAndCloneChain(unsigned ChainIndex)
{
    ...
    {
        // errs() << *TheOther << "\n"; 
        // errs() << *(TheOther->getType()) << "\n";
        // errs() << *NextInChain << "\n";
        // errs() << *(NextInChain->getType()) << "\n";
