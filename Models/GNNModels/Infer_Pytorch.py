...
def print_all_model():
  ...
  checkpoint1 = torch.load('models/Embedding_model_HARP.pth')
  print(f"Loss for HARP : {checkpoint1['loss']}")
...
if __name__ == "__main__":
  IRpath = "/home/eeuser/Desktop/GRL-HLS/Dataset/IRSet/IRTrainingSet_10/random_0.bc"
  data = graph2tensor(IR2Graph(IRpath))[0]
  edge_dim = 14
  in_channels = 145
  out_channels = 32
  checkpoint = torch.load("models/Embedding_model_HARP.pth")
  # dummy_loader = DataLoader([data], batch_size=1)  # Dummy loader for histogram computation
  harp = HARP_Pytorch.GCCGraphInfer(in_channels, out_channels, edge_dim)
  harp.load_state_dict(checkpoint['model_state_dict'])
  harp.eval()
  with torch.no_grad():
    embeddings = harp(data)
    print(embeddings)
  # print_all_model()
