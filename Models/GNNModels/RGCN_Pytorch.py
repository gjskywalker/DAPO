...
from torch_geometric.nn import PNAConv, BatchNorm, global_mean_pool, RGCNConv, summary
...
class GCCGraphInfer(nn.Module):
    def __init__(self, in_channels, out_channels, num_relations):
        ...
        self.rgcn1 = RGCNConv(in_channels, 256, num_relations)
        self.rgcn2 = RGCNConv(256, 128, num_relations)
    ...
    def forward(self, data):
        x, edge_index, edge_type, batch = data.node_attr.float(), data.edge_index, torch.tensor(np.int64(np.argmax(data.edge_attr, axis=1))), data.batch
...
if __name__ == "__main__":
    print("Starting RGCN model training")
    import os
    ...
    parser = argparse.ArgumentParser(
                    usage='python RGCN_Pytorch.py --epochs 200 --patience 3',
                    description='Train RGCN')
    ...
    in_channels = 12
    out_channels = 32
    model = GCCGraphInfer(in_channels, out_channels, num_relations=3)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_loss < best_loss:
            ...
            torch.save({
                ...
                'loss': best_loss,
                }, 'models/Embedding_model_RGCNConv.pth')
    ...
    plt.savefig('losses/RGCN_URG.png')
    plt.show()
