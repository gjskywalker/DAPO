...
from torch_geometric.nn import PNAConv, BatchNorm, global_mean_pool, SAGEConv, summary
...
class GCCGraphInfer(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.sage1 = SAGEConv(in_channels, 256)
    ...
    def forward(self, data):
        x, edge_index, batch = data.node_attr.float(), data.edge_index, data.batch
...
if __name__ == "__main__":
    print("Starting SAGE model training")
    import os
    ...
    parser = argparse.ArgumentParser(
                    usage='python SAGE_Pytorch.py --epochs 200 --patience 3',
                    description='Train SAGE')
    ...
    in_channels = 12 
    out_channels = 32
    model = GCCGraphInfer(in_channels, out_channels)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_loss < best_loss:
            ...
            torch.save({
                ...
                'loss': best_loss,
                }, 'models/Embedding_model_SAGEConv.pth')
    ...
    plt.savefig('losses/SAGE_URG.png')
    plt.show()
