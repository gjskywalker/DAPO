    ...
    while pass_len < length:
        ...
        for k in range(N):
            for cur_pass in range(total_passes):
                (passes, wall_time, sample_count) = runInsertion(
                    envs, top_passes[k], cur_pass, pool, ranked_pass_indices
                )
...
def in_test_single_pgm(N=1):
    ...
    for bm in bms:
        for length in test_len:
            ...
            i = i + 1
            begin = time.time()
            (passes, timings, sample_size) = runInsertionN(
                envs, N, length=length, sort=True
            )
            end = time.time()
            ...
            fout.write(
                "{}|{}|{}|{}|{}".format(
                    pgm, timings[0], compile_time, sample_size, passes[0]
                )
            )
...
def in_test_pgm_group(bms, N=1, test_len=[16]):
    with open("in_test_pgm_group.txt", "w") as fout:
        ...
        for length in test_len:
            ...
            fout.write(
                "{}|{}|{}|{}|{}\n".format(
                    "test_pgm_group", timings[0], compile_time, sample_size, passes[0]
                )
            )
