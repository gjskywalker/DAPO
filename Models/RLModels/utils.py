...
opt_passes = tuple(passes_18.split())
print(opt_passes)
# !!! Remember to change the idx and pgm_num when changing the testbench
...
def calculate_avg_pass_length(data: dict) -> int:
    total_length = 0
    count = 0
    for i in data:
        total_length += len(data[i]['passes'])
        count += 1
    print("Current Average Pass Length: {0}".format(total_length / count))
...
if __name__ == "__main__":
    ...
    argparse.add_argument("--calculate_avg_pass_length", type=str, default="False")
    ...
    elif boolean_string(args.calculate_avg_pass_length):
        calculate_avg_pass_length(data)
